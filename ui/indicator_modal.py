...
def init_indicator_state():
    """เตรียมค่าเริ่มต้นของอินดิเคเตอร์ทั้งหมดใน session_state"""
    for grp_list in INDICATOR_REGISTRY.values():
        for ind in grp_list:
            k = f"ind_active_{ind['code']}"
            if k not in st.session_state:
                st.session_state[k] = ind["default_active"]

@st.dialog("📚 คลังอินดิเคเตอร์และการตั้งค่า (Indicators Library)", width="large")
def show_indicators_modal():
    """แสดงหน้าต่างโมดอลสำหรับจัดการอินดิเคเตอร์ 11 ตัว"""
    init_indicator_state()

    for grp_title, ind_list in INDICATOR_REGISTRY.items():
        ...
        for ind in ind_list:
            ...
            with c_star:
                st.write("⭐" if st.session_state.get(active_key, False) else "☆")

            with c_title:
                st.markdown(f"**{name}** &nbsp; `<small style='color:#787b86;'>{{{code}}}</small>`", unsafe_allow_html=True)

            with c_sw:
                current_v = st.session_state.get(active_key, ind["default_active"])
                new_v = st.toggle("", value=current_v, key=f"modal_tgl_{code}", label_visibility="collapsed")
                if new_v != current_v:
                    st.session_state[active_key] = new_v
                    if code == "EMA": st.session_state["show_ema"] = new_v
                    elif code == "RSI": st.session_state["show_rsi_pane"] = new_v
                    elif code == "MACD": st.session_state["show_macd_pane"] = new_v
                    st.rerun()
