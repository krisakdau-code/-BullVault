...
def build_charts(df, symbol, tf, main_h=520, rsi_h=120, macd_h=120):
    ...
    # ══════════════════════════════════════════════════════════
    # 4. คำนวณอินดิเคเตอร์เพิ่มเติม (BB, Supertrend, Ichimoku, VWAP, Stoch, ATR, ADX, Vol)
    # ══════════════════════════════════════════════════════════
    ...
    crosshair_synced = {
        "mode": 1,
        "vertLine": {"visible": True, "style": 3, "width": 1, "color": "rgba(255, 255, 255, 0.40)", "labelVisible": True, "labelBackgroundColor": theme.BORDER_COLOR},
        "horzLine": {"visible": True, "style": 3, "width": 1, "color": "rgba(255, 255, 255, 0.40)", "labelVisible": True, "labelBackgroundColor": theme.BORDER_COLOR},
    }

    base_chart = {
        "layout": {"background": {"type": "solid", "color": theme.CHART_BG}, "textColor": theme.TEXT_COLOR, "fontSize": 11},
        "grid": {"vertLines": {"color": theme.GRID_COLOR, "style": 1}, "horzLines": {"color": theme.GRID_COLOR, "style": 1}},
        "crosshair": crosshair_synced,
        "rightPriceScale": {"autoScale": True, "borderColor": theme.BORDER_COLOR, "borderVisible": True, "scaleMargins": {"top": theme.PRICE_SCALE_TOP, "bottom": theme.PRICE_SCALE_BTM}, "mode": 0},
        "overlayPriceScales": {"scaleMargins": {"top": theme.VOL_TOP_MARGIN, "bottom": 0.0}},
        "timeScale": pane_ts,
    ...
    price_series = [{
        ...
        "data": candles,
        "options": {
            "upColor": theme.CANDLE_UP, "downColor": theme.CANDLE_DOWN, "borderVisible": True,
            "borderUpColor": theme.CANDLE_UP, "borderDownColor": theme.CANDLE_DOWN,
            "wickUpColor": theme.CANDLE_UP, "wickDownColor": theme.CANDLE_DOWN,
            "priceScaleId": "right",
            "priceFormat": {"type": "price", "precision": price_precision(d), "minMove": min_move(d)},
        },
    }]
    ...
    if is_timeframe_visible(tf):
        if st.session_state.get("ind_active_RSI", st.session_state.get("show_rsi_pane", True)): charts.append(make_rsi_pane())
        if st.session_state.get("ind_active_MACD", st.session_state.get("show_macd_pane", True)): charts.append(make_macd_pane())
        if st.session_state.get("ind_active_STOCH", False): charts.append(make_stoch_pane())
        if st.session_state.get("ind_active_ATR", False): charts.append(make_atr_pane())
        if st.session_state.get("ind_active_ADX", False): charts.append(make_adx_pane())
        if st.session_state.get("ind_active_VOL", False): charts.append(make_vol_pane())
